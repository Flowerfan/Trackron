...
from trackron.structures import Boxes, BoxMode, pairwise_iou
from trackron.utils.logger import create_small_table
...
class DETEvaluator(DatasetEvaluator):
    ...
    """
        Args:
            dataset_name (str): name of the dataset to be evaluated.
                ...
                Or it must be in trackron's standard dataset format
  ...
  def process(self, inputs, outputs):
    ...
    if self._coco_api is None:
      self._coco_api = COCO(inputs.coco_path)
    assert 'detect_out' in outputs and 'frames' in outputs  ## must contain necessary fields
    for idx, frame_info in enumerate(outputs['frames']):
      if len(frame_info) < 1: continue ### only process key frame
      image_id = frame_info['id']
      prediction = {"image_id": image_id}
      output = outputs['detect_out'][idx]
      prediction["instances"] = prediction_to_coco_json(
          output, image_id)
  ...
  def _eval_predictions(self, predictions, img_ids=None):
    ...
    for task in sorted(tasks):
      ...
      res = self._derive_coco_results(coco_eval, task, class_names=None)
...
def prediction_to_coco_json(pred_dict, img_id):
  """
    ...
    """
  num_prediction = len(pred_dict['scores'])
  if num_prediction == 0:
    return []

  boxes = box_convert(pred_dict['boxes'], 'xyxy', 'xywh')
  boxes = boxes.cpu().numpy().tolist()
  scores = pred_dict['scores'].tolist()
  classes = pred_dict['labels'].tolist()

  has_mask = pred_dict.get("pred_masks", False)
  if has_mask:
    # use RLE to encode the masks, because they are too large and takes memory
    # since this evaluator stores outputs of the entire dataset
    rles = [
        mask_util.encode(np.array(mask[:, :, None], order="F",
                                  dtype="uint8"))[0]
        for mask in pred_dict['pred_masks']
    ]
    for rle in rles:
  ...
  has_keypoints = pred_dict.get('pred_keypoints', None)
  if has_keypoints:
    keypoints = pred_dict['pred_keypoints']

  results = []
  for k in range(num_prediction):
    result = {
        "image_id": img_id,
