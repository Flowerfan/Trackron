...
from trackron.structures import Sequence
from trackron.trackers import TrackingActor
from trackron.utils.misc import imwrite_indexed


def _save_tracker_output(seq: Sequence, tracker: TrackingActor, output: dict):
  ...
  for key, data in output.items():
    ...
    if key == 'target_bbox':
      ...
      for frame_name, frame_seg in zip(frame_names, data):
        imwrite_indexed(
            os.path.join(segmentation_path, '{}.png'.format(frame_name)),
            frame_seg)


def run_sequence(seq: Sequence,
                 tracker: TrackingActor,
                 debug=False,
                 visdom_info=None):
  ...
  print('Tracker: {} {} ,  Sequence: {}'.format(tracker.name, tracker.run_id,
                                                seq.name))
