...
from trackron.data.datasets.testsets.load_text import load_text
...
def extract_results(trackers, dataset, result_dir, report_name, skip_missing_seq=False, plot_bin_gap=0.05,
                    exclude_invalid_frames=False):
    """[summary]

    Args:
        trackers ([type]): [description]
        dataset ([type]): [description]
        result_dir ([type]): [description]
        report_name ([type]): [description]
        skip_missing_seq (bool, optional): [description]. Defaults to False.
        plot_bin_gap (float, optional): [description]. Defaults to 0.05.
        exclude_invalid_frames (bool, optional): [description]. Defaults to False.
    ...
    if not result_plot_path.exists():
        result_plot_path.mkdir(parents=True)
    ...
    tracker_names = [{'name': t.name, 'run_id': t.run_id, 'disp_name': t.display_name}
                     for t in trackers]
