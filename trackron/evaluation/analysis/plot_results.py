...
def get_plot_draw_styles():
  plot_draw_style = [{
      ...
      'color': (136.0 / 255.0, 0.0, 21.0 / 255.0),
      'line_style': '-'
  }, {
      'color': (1.0, 127.0 / 255.0, 39.0 / 255.0),
      'line_style': '-'
  }, {
      'color': (0.0, 162.0 / 255.0, 232.0 / 255.0),
      'line_style': '-'
  }, {
      'color': (0.0, 0.5, 0.0),
      'line_style': '-'
...
def check_eval_data_is_valid(eval_data, trackers, dataset):
  ...
  tracker_names_f = [(t.name, t.run_id) for t in trackers]
  tracker_names_f_saved = [
      (t['name'], t['run_id']) for t in eval_data['trackers']
  ]
...
def merge_multiple_runs(eval_data):
  ...
  for i in range(len(trackers)):
    ...
    match = [
        t['name'] == base_tracker['name'] for t in trackers
    ]
  ...
  eval_data[
      'ave_success_rate_plot_overlap'] = ave_success_rate_plot_overlap_merged.tolist(
      )
  eval_data[
      'ave_success_rate_plot_center'] = ave_success_rate_plot_center_merged.tolist(
      )
  eval_data[
      'ave_success_rate_plot_center_norm'] = ave_success_rate_plot_center_norm_merged.tolist(
      )
...
def get_tracker_display_name(tracker):
  if tracker['disp_name'] is None:
    if tracker['run_id'] is None:
      disp_name = '{}'.format(tracker['name'])
    else:
      disp_name = '{}_{:03d}'.format(tracker['name'], tracker['run_id'])
  else:
...
def plot_draw_save(y, x, scores, trackers, plot_draw_styles, result_plot_path,
  ...
  tikzplotlib.save(result_plot_path / plot_type)
  fig.savefig(result_plot_path / '{}_plot.pdf'.format(plot_type),
              dpi=300,
              format='pdf',
              transparent=True)
  plt.draw()
...
def check_and_load_precomputed_results(trackers,
  ...
  result_plot_path = Path(result_dir) / report_name
  eval_data_path = result_plot_path / 'eval_data.pkl'

  if eval_data_path.is_file() and not force_evaluation:
    with eval_data_path.open('rb') as fh:
      eval_data = pickle.load(fh)
  else:
    # print('Pre-computed evaluation data not found. Computing results!')
    eval_data = extract_results(trackers, dataset, result_dir, report_name,
                                **kwargs)
...
def plot_results(trackers,
  ...
  # Load pre-computed results
  result_plot_path = Path(result_dir) / report_name
  eval_data = check_and_load_precomputed_results(trackers, dataset, result_dir,
                                                 report_name, force_evaluation,
                                                 **kwargs)
  ...
  if 'prec' in plot_types:
    ...
    plot_draw_save(prec_curve, threshold_set_center, prec_score, tracker_names,
                   plot_draw_styles, result_plot_path, precision_plot_opts)
...
def generate_formatted_report(row_labels, scores, table_name=''):
  ...
  for trk_id, d_name in enumerate(row_labels):
    # display name
    report_text = '{prev}{tracker: <{width}} |'.format(prev=report_text,
                                                       tracker=d_name,
                                                       width=name_width)
...
def print_results(trackers,
  ...
  """ Print the results for the given trackers in a formatted table
    args:
        trackers - List of trackers to evaluate
        dataset - List of sequences to evaluate
        report_name - Name of the folder where the computed results and plots are saved
        merge_results - If True, multiple random runs for a non-deterministic trackers are averaged
  ...
  report_text = generate_formatted_report(tracker_disp_names,
                                          scores,
                                          table_name=report_name)
  print(report_text)


def plot_got_success(trackers, result_dir, report_name):
  ...
  for trk_id, trk in enumerate(trackers):
    json_path = result_plot_path / '{}.json'.format(trk.name)
    # json_path = '{}/{}.json'.format(settings.got_reports_path, trk.name)

    if json_path.is_file():
      with json_path.open(model="r") as f:
...
def print_per_sequence_results(trackers,
  ...
  """ Print per-sequence results for the given trackers. Additionally, the sequences to list can be filtered using
    ...
    """
  # Load pre-computed results
  eval_data = check_and_load_precomputed_results(trackers, dataset, report_name,
                                                 **kwargs)
