...
class ImageList(object):
  ...
  def __init__(self, tensor: torch.Tensor, image_sizes: List[Tuple[int, int]], mask: Optional[torch.Tensor]=None):
    """
        Arguments:
  ...
  def from_tensors(tensors: List[torch.Tensor],
                   ...
                   pad_value: float = 0.0,
                   masks: Optional[List[torch.Tensor]] = None) -> "ImageList":
    """
        Args:
    ...
    if masks is not None:
      ...
      assert tensors[0].shape[-2:] == masks[0].shape[-2:]
    for t in tensors:
    ...
    if len(tensors) == 1:
      ...
      if masks is not None:
        batched_masks = F.pad(masks[0], padding_size, value=1.0).unsqueeze_(0)
    else:
      # max_size can be a tensor in tracing mode, therefore convert to list
      batch_shape = [len(tensors)] + list(
          tensors[0].shape[:-2]) + list(max_size)
      batched_imgs = tensors[0].new_full(batch_shape, pad_value)
      batched_masks = masks[0].new_full((len(masks), *masks[0].shape[:-2], *max_size), 1.0) if masks is not None else None
      for idx, (img, pad_img) in enumerate(zip(tensors, batched_imgs)):
        pad_img[..., :img.shape[-2], :img.shape[-1]].copy_(img)
        if batched_masks is not None:
          batched_masks[idx, ..., :img.shape[-2], :img.shape[-1]].copy_(masks[idx])
