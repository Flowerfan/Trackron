...
from trackron.utils.memory import retry_if_cuda_oom
...
class BitMasks:
  ...
  def __init__(self, tensor: Union[torch.Tensor, np.ndarray]):
    """
        Args:
            tensor: bool Tensor of N,H,W, representing N instances in the image.
        """
    device = tensor.device if isinstance(tensor,
                                         torch.Tensor) else torch.device("cpu")
  ...
  def cat(bitmasks_list: List["BitMasks"]) -> "BitMasks":
    ...
    cat_bitmasks = type(bitmasks_list[0])(torch.cat(
...
class PolygonMasks:
  ...
  def __getitem__(
    ...
    if isinstance(item, int):
      ...
      if item.dtype == torch.bool:
        ...
        raise ValueError("Unsupported tensor dtype={} for indexing!".format(
            item.dtype))
      selected_polygons = [self.polygons[i] for i in item]
  ...
  def cat(polymasks_list: List["PolygonMasks"]) -> "PolygonMasks":
    ...
    cat_polymasks = type(polymasks_list[0])(list(
