...
class TAO:
  ...
  def __init__(self, annotation_path):
    ...
    if isinstance(annotation_path, dict):
      for key in ('info', 'images', 'annotations', 'categories', 'videos',
                  'tracks'):
  ...
  def _create_index(self):
    ...
    if negative_anns:
      logger.warn(f'{len(negative_anns)} annotations had negative '
                       f'values in coordinates!')
      logger.debug('Annotation ids with negative values:\n' +
  ...
  def get_ann_ids(self,
    ...
    if vid_ids is not None:
      ...
      for video_id in vid_ids:
        video_images.extend([x['id'] for x in self.vid_img_map[video_id]])
      if img_ids is None:
    ...
    ann_ids = [
        _ann["id"] for _ann in anns if _ann["category_id"] in cat_ids and
        _ann["area"] > area_rng[0] and _ann["area"] < area_rng[1]
    ]
    return ann_ids
