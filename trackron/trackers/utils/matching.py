...
import trackron.trackers.utils.kalman_filter as kalman_filter
...
def fuse_motion(kf,
  ...
  for row, track in enumerate(tracks):
    ...
    cost_matrix[row] = lambda_ * cost_matrix[row] + (1 -
                                                     lambda_) * gating_distance
  return cost_matrix
...
def category_gate(cost_matrix, tracks, detections):
  ...
  cost_matrix = cost_matrix + np.abs(det_categories[None, :] -
                                     trk_categories[:, None])
  return cost_matrix
...
def fuse_score(cost_matrix, detections):
  ...
  det_scores = np.expand_dims(det_scores, axis=0).repeat(cost_matrix.shape[0],
                                                         axis=0)
