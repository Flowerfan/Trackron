...
from trackron.structures import TensorList
from trackron.utils.plotting import show_tensor, plot_graph
from trackron.libs import dcf
from trackron.models.cls_heads.filter import FilterInitializerZero
from trackron.models.layers import activation

from .base_tracker import BaseTracker
from .utils.preprocessing import sample_patch_multiscale, sample_patch_transformed, numpy_to_torch
...
class DiMPTracker(BaseTracker):
  ...
  def init_sot(self, image, info: dict) -> dict:
    ...
    # Convert image
    im = numpy_to_torch(image).to(self.device)

    # Get target position and size
    state = info['init_bbox']
    self.pos = torch.Tensor(
        [state[1] + (state[3] - 1) / 2,
         state[0] + (state[2] - 1) / 2]).to(self.device)
    self.target_sz = torch.Tensor([state[3], state[2]]).to(self.device)
    ...
    # Set sizes
    self.image_sz = torch.Tensor([im.shape[2], im.shape[3]]).to(self.device)
    sz = self.params.image_sample_size
    sz = torch.Tensor([sz, sz] if isinstance(sz, int) else sz).to(self.device)
    ...
    if not self.params.has('scale_factors'):
      self.params.scale_factors = torch.ones(1).to(self.device)
    elif isinstance(self.params.scale_factors, (list, tuple)):
      self.params.scale_factors = torch.Tensor(self.params.scale_factors).to(
          self.device)
    ...
    self.box2box_transform = boxtrans.Box2BoxTransform([10.0, 10.0, 5.0, 5.0])

    # Initialize IoUNet
    if self.params.get('use_iou_net', True):
      self.init_iou_net(init_backbone_feat)

    out = {'time': time.time() - tic, 'target_bbox': state}
    if self.params.get('output_score', False):
      out['target_bbox'] = state + [1.0]
    self.score_maps = []
    return out
  ...
  def track_sot(self, image, info: dict = None) -> dict:
    ...
    if flag != 'not_found':
      if self.params.get('use_iou_net', True):
        update_scale_flag = self.params.get('update_scale_when_uncertain',
    ...
    if self.params.get('use_iou_net', True) and flag != 'not_found' and hasattr(
        self, 'pos_iounet'):
      self.pos = self.pos_iounet.clone()

    score_map = s[scale_ind, ...]
    # max_score = torch.max(score_map).item()
    max_score = torch.max(scores_raw).item()

    # Visualize and set debug info
    self.search_area_box = torch.cat(
        (sample_coords[scale_ind, [1, 0]], sample_coords[scale_ind, [3, 2]] -
         sample_coords[scale_ind, [1, 0]] - 1))
    self.debug_info['flag' + self.id_str] = flag
    self.debug_info['max_score' + self.id_str] = max_score
    self.score_maps += [score_map.cpu().numpy()]
    ...
    new_state = torch.cat((self.pos[[1, 0]] - (self.target_sz[[1, 0]] - 1) / 2,
                           self.target_sz[[1, 0]]))
    ...
    if self.params.get('output_score', False):
      output_state = output_state + [max_score]
  ...
  def get_sample_location(self, sample_coord):
    ...
    sample_scales = ((sample_coord[:, 2:] - sample_coord[:, :2]) /
  ...
  def classify_target(self, sample_x: TensorList):
    ...
    with torch.no_grad():
      scores = self.net.cls_head.classify(self.target_filter, sample_x)
    return scores
  ...
  def localize_target(self, scores, sample_pos, sample_scales):
    ...
    if preprocess_method is None:
      ...
      reg_val = getattr(self.net.cls_head.filter_optimizer, 'softmax_reg', None)
    ...
    # Get maximum
    score_sz = torch.Tensor(list(scores.shape[-2:])).to(self.device)
  ...
  def localize_advanced(self, scores, sample_pos, sample_scales):
    ...
    sz = scores.shape[-2:]
    score_sz = torch.Tensor(list(sz)).to(self.device)
    output_sz = score_sz - (self.kernel_size + 1) % 2
    score_center = (score_sz - 1) / 2

    scores_hn = scores
    if self.output_window is not None and self.params.get(
        'perform_hn_without_windowing', False):
    ...
    # max_disp1 = max_disp1[scale_ind, ...].float().view(-1)
    max_disp1 = max_disp1[scale_ind].float().view(-1)
    ...
    if max_score1.item() < self.params.get('uncertain_threshold',
                                           -float('inf')):
      return translation_vec1, scale_ind, scores_hn, 'uncertain'
    if max_score1.item() < self.params.get('hard_sample_threshold',
                                           -float('inf')):
    ...
    tneigh_top = max(round(max_disp1[0].item() - target_neigh_sz[0].item() / 2),
                     0)
    tneigh_bottom = min(
    ...
    max_disp2 = max_disp2.float().view(-1)
  ...
  def extract_backbone_features(self, im: torch.Tensor, pos: torch.Tensor,
    ...
    with torch.no_grad():
      im_patches /= 255.0
      im_patches = self.net.preprocess_image(im_patches.to(self.device))
      backbone_feat = self.net.extract_backbone_features(im_patches)
  ...
  def get_iou_features(self, backbone_feat):
    with torch.no_grad():
      return self.net.box_head.get_iou_feat(
          self.get_iou_backbone_features(backbone_feat))

  def get_iou_modulation(self, iou_backbone_feat, target_boxes):
    with torch.no_grad():
      return self.net.box_head.get_modulation(iou_backbone_feat, target_boxes)
  ...
  def generate_init_samples(self, im: torch.Tensor) -> TensorList:
    ...
    if mode == 'inside':
      ...
      im_sz = torch.Tensor([im.shape[2], im.shape[3]]).to(self.device)
      ...
      shrink_factor.clamp_(min=1,
                           max=self.params.get('patch_max_scale_change', None))
      ...
      global_shift = -((-tl).clamp(0) -
                       (br - im_sz).clamp(0)) / self.init_sample_scale
    else:
    ...
    aug_expansion_factor = self.params.get('augmentation_expansion_factor',
                                           None)
    ...
    if random_shift_factor > 0:
      get_rand_shift = lambda: ((torch.rand(2) - 0.5) * self.img_sample_sz.cpu(
      ) * random_shift_factor + global_shift).long().tolist()
    ...
    augs = self.params.augmentation if self.params.get('use_augmentation',
                                                       True) else {}
    ...
    if 'relativeshift' in augs:
      get_absolute = lambda shift: (torch.Tensor(shift) * self.img_sample_sz.
                                    cpu() / 2).long().tolist()
      self.transforms.extend([
          augmentation.Translation(get_absolute(shift),
                                   aug_output_sz,
                                   global_shift.long().tolist(),
                                   device=self.device)
    ...
    if 'blur' in augs:
      self.transforms.extend([
          augmentation.Blur(sigma,
                            aug_output_sz,
                            get_rand_shift(),
                            device=self.device) for sigma in augs['blur']
      ])
    if 'scale' in augs:
      self.transforms.extend([
          augmentation.Scale(scale_factor,
                             aug_output_sz,
                             get_rand_shift(),
                             device=self.device)
    ...
    if 'rotate' in augs:
      self.transforms.extend([
          augmentation.Rotate(angle,
                              aug_output_sz,
                              get_rand_shift(),
                              device=self.device) for angle in augs['rotate']
      ])

    # Extract augmented image patches
    im_patches = sample_patch_transformed(im, self.init_sample_pos,
    ...
    with torch.no_grad():
      im_patches /= 255.0
      im_patches = self.net.preprocess_image(im_patches)
      init_backbone_feat = self.net.extract_backbone_features(im_patches)

    return init_backbone_feat

  def init_target_boxes(self):
    """Get the target bounding boxes for the initial augmented samples."""
    self.classifier_target_box = self.get_iounet_box(self.pos, self.target_sz,
                                                     self.init_sample_pos,
                                                     self.init_sample_scale)
    init_target_boxes = TensorList()
    for T in self.transforms:
      init_target_boxes.append(
          self.classifier_target_box +
          torch.Tensor([T.shift[1], T.shift[0], 0, 0]).to(self.device))
    init_target_boxes = torch.cat(
        init_target_boxes.view(1, 4).to(self.device), 0)
  ...
  def finish(self):
    '''set box_head grads to true for future training'''
    for p in self.net.box_head.parameters():
  ...
  def init_iou_net(self, backbone_feat):
    ...
    target_boxes = torch.cat(target_boxes.view(1, 4).to(self.device), 0)
  ...
  def init_classifier(self, init_backbone_feat):
    ...
    self.feature_sz = torch.Tensor(list(x.shape[-2:])).to(self.device)
    ksz = self.params.filter_size
    self.kernel_size = torch.Tensor([ksz, ksz] if isinstance(ksz, (
        int, float)) else ksz).to(self.device)
    ...
    if plot_loss:
      ...
      if self.visdom is not None:
        self.visdom.register((self.losses, torch.arange(self.losses.numel())),
  ...
  def _overwrite_classifier_params(self, feature_dim):
    # Overwrite some parameters in the classifier. (These are not generally changed)
    pred_module = getattr(self.net.cls_head.filter_optimizer, 'score_predictor',
                          self.net.cls_head.filter_optimizer)
    if self.params.get('label_threshold', None) is not None:
      self.net.cls_head.filter_optimizer.label_threshold = self.params.label_threshold
    if self.params.get('label_shrink', None) is not None:
      self.net.cls_head.filter_optimizer.label_shrink = self.params.label_shrink
    if self.params.get('softmax_reg', None) is not None:
      self.net.cls_head.filter_optimizer.softmax_reg = self.params.softmax_reg
    ...
    if self.params.get('filter_init_zero', False):
      self.net.cls_head.filter_initializer = FilterInitializerZero(
          self.net.cls_head.filter_size, feature_dim)
  ...
  def update_classifier(self,
    ...
    if num_iter > 0:
      ...
      feat_center = (target_boxes[..., :2] +
                     target_boxes[..., 2:] / 2) / self.params.feature_stride

      # Run the filter optimizer module
      with torch.no_grad():
        self.target_filter, _, losses = self.net.cls_head.filter_optimizer(
            self.target_filter,
            num_iter=num_iter,
            feat=samples,
            center=feat_center,
  ...
  def refine_target_box(self,
    ...
    if hasattr(self.net.box_head, 'predict_bb'):
      return self.direct_box_regression(backbone_feat, sample_pos, sample_scale,
                                        scale_ind, update_scale)
    ...
    if self.params.num_init_random_boxes > 0:
      ...
      rand_bb = (torch.rand(self.params.num_init_random_boxes, 4) - 0.5).to(
          self.device) * rand_factor
  ...
  def optimize_boxes_default(self, iou_features, init_boxes):
    ...
    output_boxes = init_boxes.view(1, -1, 4).to(self.device)
    step_length = self.params.box_refinement_step_length
    if isinstance(step_length, (tuple, list)):
      step_length = torch.Tensor(
          [step_length[0], step_length[0], step_length[1],
           step_length[1]]).to(self.device).view(1, 1, 4)
    ...
    for i_ in range(self.params.box_refinement_iter):
      ...
      outputs = self.net.box_head.predict_iou(self.iou_modulation, iou_features,
                                              bb_init)
  ...
  def direct_box_regression(self,
                            backbone_feat,
                            sample_pos,
                            sample_scale,
                            scale_ind,
    ...
    # Generate random initial boxes
    init_boxes = init_box.view(1, 1, 4).clone().to(self.device)

    # Optimize the boxes
    for i in range(1):

      bb_reg = self.net.box_head.predict_bb(self.iou_modulation, iou_features,
                                            init_boxes)
      # output_boxes = (init_boxes + bb_reg).view(-1,4).cpu()
      output_boxes = self.box2box_transform.apply_deltas(
          bb_reg.view(-1, 4), init_boxes.view(-1, 4))
      init_boxes = output_boxes.reshape(1, 1, 4)
    output_boxes = output_boxes.cpu()
    ...
    new_pos = (new_pos.flip(
        (0,)) - (self.img_sample_sz - 1) / 2) * sample_scale + sample_pos
    new_target_sz = predicted_box[2:].flip((0,)) * sample_scale
    new_scale_bbr = torch.sqrt(new_target_sz.prod() /
  ...
  def visualize_iou_pred(self, iou_features, center_box):
    ...
    pos_boxes = bbutils.rel_to_rect((center_box_rel + bbx) + bby,
                                    sz_norm).view(1, -1, 4).to(self.device)
    sz_boxes = bbutils.rel_to_rect((center_box_rel + bbw) + bbh,
                                   sz_norm).view(1, -1, 4).to(self.device)

    pos_scores = self.net.box_head.predict_iou(self.iou_modulation,
                                               iou_features, pos_boxes).exp()
    sz_scores = self.net.box_head.predict_iou(self.iou_modulation, iou_features,
                                              sz_boxes).exp()

    show_tensor(pos_scores.view(pos_scale.numel(), -1),
                title='Position scores',
                fig_num=21)
    show_tensor(sz_scores.view(sz_scale.numel(), -1),
                title='Size scores',
                fig_num=22)

  # def visdom_draw_tracking(self, image, out, segmentation=None, vwriter=None):
  #   box = out['target_box']
  #   if hasattr(self, 'search_area_box'):
  #     self.visdom.register((image, box, self.search_area_box), 'Tracking', 1,
  #                          'Tracking')
  ...
  #       draw.rectangle((tuple(box[:2]), (box[0] + box[2], box[1] + box[3])),
  #                      outline='green',
  #                      width=5)
  #       sbox = self.search_area_box
  #       draw.rectangle(
  #           ((sbox[0], sbox[1]), (sbox[0] + sbox[2], sbox[1] + sbox[3])),
  #           outline='red',
  #           width=5)
  #       vwriter.writeFrame(np.array(im))
  #   else:
  #     self.visdom.register((image, box), 'Tracking', 1, 'Tracking')
