...
class OTBDatasetDict(BaseDataset):
    """ OTB-2015 dataset
    ...
    def _construct_sequence(self, sequence_info):
        ...
        # return Sequence(sequence_info['name'], frames, 'otbdict', ground_truth_rect[init_omit:,:],
        #                 object_class=sequence_info['object_class'])
        init_data = nested_dict()
        object_ids = [0]
        # for idx in range(end_frame-init_omit):
        init_data[0]['bbox'][0] = ground_truth_rect[init_omit].astype(np.float32)
        gt_bboxes = init_data
        return Sequence(sequence_info['name'], frames, 'otbdict', gt_bboxes, init_data=init_data, object_ids=object_ids, multiobj_mode=True)
