...
class TAO(BaseVideoDataset):
    ...
    def __init__(self, root, image_loader=jpeg4py_loader, data_fraction=None, split="train"):
        """
        args:
        ...
        self.img_pth = os.path.join(
            root, 'frames')
        self.anno_path = os.path.join(
            root, 'annotations-1.1/{}.json'.format(split))
    ...
    def _get_sequence_list(self):
        ...
        for track in self.tracks:
            ...
            seq_infos = {'num_imgs': len(img_ids), 'img_ids': img_ids, 
                         'track_id': track_id, 'anns':[], 'video_id': video_id,
    ...
    def get_frames(self, seq_id=None, frame_ids=None, anno=None):
        frame_infos = self.tao.loadImgs(sorted(self.sequence_list[seq_id]['img_ids']))
        frame_list = [self._get_frame(frame_infos[fid]['file_name'])
