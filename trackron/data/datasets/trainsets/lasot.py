...
class Lasot(BaseVideoDataset):
  ...
  def __init__(self, root, image_loader=jpeg4py_loader, vid_ids=None, split=None, data_fraction=None):
    """
        args:
  ...
  def get_sequence_info(self, seq_id):
    ...
    valid = (bbox[:, 0] >= 0) & (bbox[:, 1] >= 0) & (bbox[:, 2] > 0) & (bbox[:, 3] > 0) & (
        bbox[:, 0] + bbox[:, 2] <= width) & (bbox[:, 1] + bbox[:, 3] <= height)
