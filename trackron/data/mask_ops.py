...
def crop_and_resize(tensor: torch.Tensor, boxes: torch.Tensor,
  ...
  """
      ...
      Args:
          tensor (Tensor): NxMxN tensor storing mask
      ...
      """
  assert len(boxes) == len(tensor), "{} != {}".format(len(boxes), len(tensor))
  ...
  bit_masks = tensor.to(dtype=torch.float32)
...
def paste_masks_in_image(masks: torch.Tensor,
                         boxes: Boxes,
  ...
  assert masks.shape[-1] == masks.shape[
      -2], "Only square mask predictions are supported"
  N = len(masks)
  if N == 0:
