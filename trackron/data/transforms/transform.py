...
class RotationTransform(Transform):
  ...
  def __init__(self, h, w, angle, expand=True, center=None, interp=None):
    ...
    if expand:
      ...
      bound_w, bound_h = np.rint(
          [h * abs_sin + w * abs_cos, h * abs_cos + w * abs_sin]).astype(int)
    else:
      bound_w, bound_h = w, h
  ...
  def create_rotation_matrix(self, offset=0):
    ...
    if self.expand:
      ...
      rot_im_center = cv2.transform(self.image_center[None, None, :] + offset,
                                    rm)[0, 0, :]
  ...
  def inverse(self):
    ...
    rotation = RotationTransform(self.bound_h, self.bound_w, -self.angle, True,
                                 None, self.interp)
