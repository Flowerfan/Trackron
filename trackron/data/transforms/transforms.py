...
class RandomHorizontalFlip(TransformBase):
  ...
  def transform_bbox(self, boxes, image_shape, do_flip):
    ...
    if do_flip:
      ...
      boxes = boxes.clone()
      boxes = torch.stack([
          w - boxes[..., 0] - boxes[..., 2], boxes[..., 1], boxes[..., 2],
          boxes[..., 3]
      ],
                          dim=-1)
    return boxes
...
class Blur(TransformBase):
  ...
  def __init__(self, sigma):
    ...
    self.filter = [
        torch.exp(-(x**2) / (2 * s**2)) for x, s in zip(x_coord, self.sigma)
    ]
  ...
  def transform_image(self, image):
    if torch.is_tensor(image):
      ...
      return F.conv2d(im1, self.filter[1],
                      padding=(0, self.filter_size[1])).view(-1, sz[0], sz[1])
    else:
  ...
  def transform(self, image, do_blur=None):
    ...
    if do_blur:
      if torch.is_tensor(image):
        ...
        return F.conv2d(im1, self.filter[1],
                        padding=(0,
                                 self.filter_size[1])).view(-1, sz[0], sz[1])
      else:
        raise NotImplementedError
    else:
      return image
...
class RandomAffine(TransformBase):
  ...
  def transform_image(self, image, do_flip, theta, shear_values, scale_factors):
    ...
    t_mat = self._construct_t_mat(image.shape[:2], do_flip, theta, shear_values,
                                  scale_factors)
  ...
  def transform_coords(self, coords, image_shape, do_flip, theta, shear_values,
                       scale_factors):
    t_mat = self._construct_t_mat(image_shape, do_flip, theta, shear_values,
                                  scale_factors)
  ...
  def transform_mask(self, mask, do_flip, theta, shear_values, scale_factors):
    t_mat = self._construct_t_mat(mask.shape[:2], do_flip, theta, shear_values,
                                  scale_factors)
...
class ResizeShortestEdge(TransformBase):
  ...
  def __call__(self, **inputs):
    ...
    for var_name, var in input_vars.items():
      if var is not None:
        transform_func = getattr(self, 'transform_' + var_name)
        outputs[var_name] = transform_func(var, image_size, self.resize)
    return outputs
...
class RandomSizeCrop(TransformBase):
  ...
  def __call__(self, **inputs):
    ...
    image_size = self._get_image_size(input_vars)
    if input_args.get('new_roll', True):
      h = random.randint(self.min_size, min(image_size[0], self.max_size))
      w = random.randint(self.min_size, min(image_size[1], self.max_size))
