...
class ResizeScale(Augmentation):
  ...
  def get_transform(self, image: np.ndarray) -> Transform:
    ...
    scale = np.minimum(random_scale_size[0] / input_size[0],
                       random_scale_size[1] / input_size[1])
    scaled_size = np.round(np.multiply(input_size, scale)).astype(int)
    return ResizeTransform(input_size[0], input_size[1], scaled_size[0],
                           scaled_size[1], self.interp)
...
class FixedSizeCrop(Augmentation):
  ...
  def get_transform(self, image: np.ndarray) -> TransformList:
    ...
    crop_transform = CropTransform(offset[1], offset[0], output_size[1],
                                   output_size[0], input_size[1], input_size[0])
  ...
  def get_transform(self, image, sem_seg):
    if self.single_category_max_area >= 1.0:
      ...
      for _ in range(10):
        ...
        if len(cnt) > 1 and np.max(
            cnt) < np.sum(cnt) * self.single_category_max_area:
          break
...
class RandomLighting(Augmentation):
  ...
  def __init__(self, scale):
    ...
    super().__init__()
    self._init(locals())
    self.eigen_vecs = np.array([[-0.5675, 0.7192, 0.4009],
  ...
  def get_transform(self, image):
    ...
    return BlendTransform(src_image=self.eigen_vecs.dot(weights *
                                                        self.eigen_vals),
                          src_weight=1.0,
                          dst_weight=1.0)
