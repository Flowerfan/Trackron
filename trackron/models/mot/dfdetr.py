...
class DFDETRHEAD(nn.Module):
  ...
  def __init__(self,
               *,
               ...
               encoder: nn.Module,
               detect_decoder: nn.Module,
               track_decoder: nn.Module,
    ...
    self.detect_decoder = detect_decoder
  ...
  def from_config(cls, cfg, in_feature_channels):
    ...
    detect_decoder = DeformableTransformerDecoder(decoder_layer,
                                                  cfg.DECODER.NUM_LAYERS, True)
    track_decoder = DeformableTransformerDecoder(decoder_layer,
                                                 cfg.DECODER.NUM_LAYERS, True)
    ...
    if cfg.BOX_REFINE:
      ...
      detect_decoder.bbox_embed = box_head
    else:
  ...
  def forward(self, temp_feat, search_feat, temp_mask, search_mask):
    ...
    temp_logits, temp_boxes = self.decode_mot_cls_box(temp_target_emb, init_ref,
                                                      inter_ref)

    #### track
    search_target_emb, inter_ref = self.track_decoder(temp_target_emb[-1],
                                                      temp_boxes[-1],
                                                      search_memory,
                                                      spatial_shapes,
                                                      valid_ratios,
                                                      None,
                                                      search_masks)
    track_logits, track_boxes = self.decode_mot_cls_box(search_target_emb,
                                                        temp_boxes[-1],
                                                        inter_ref)
  ...
  def decode_targets(self, features, masks, spatial_shapes, valid_ratios):
    ...
    hs, inter_references = self.detect_decoder(tgt, reference_points, features,
  ...
  def track(self, feature, mask, pre_embed=None, targets=None):
    ...
    if pre_embed is not None:
      ...
      search_target_emb, inter_ref = self.track_decoder(pre_tgt,
                                                        pre_reference,
                                                        memory,
                                                        spatial_shapes,
                                                        valid_ratios,
      ...
      track_logits, track_boxes = self.decode_mot_cls_box(search_target_emb,
                                                          pre_reference,
                                                          inter_ref)
      track_logits, track_boxes = track_logits[-1], track_boxes[-1]

    else:
      track_logits = cur_class
      track_boxes = cur_box

    out = {
        'pred_logits': cur_class,
        'pred_boxes': cur_box,
        'tracking_logits': track_logits,
