...
class Transformer(nn.Module):
    def __init__(self, d_model=512, nhead=1, num_layers=1, dim_feedforward=2048,
        ...
        self.encoder = TransformerEncoder(
            multihead_attn=multihead_attn, FFN=None, d_model=d_model, num_encoder_layers=num_layers)
        self.decoder = TransformerDecoder(
            multihead_attn=multihead_attn, FFN=None, d_model=d_model, num_decoder_layers=num_layers)
    ...
    def forward(self, train_feat, test_feat, train_label):
        ...
        for i in range(num_img_train):
            _, cur_encoded_feat = self.decoder(train_feat[i, ...].unsqueeze(
                0), memory=encoded_memory, pos=train_label, query_pos=None)
            if i == 0:
        ...
        for i in range(num_img_test):
            _, cur_decoded_feat = self.decoder(test_feat[i, ...].unsqueeze(
                0), memory=encoded_memory, pos=train_label, query_pos=None)
            if i == 0:
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, src, pos: Optional[Tensor] = None):
        ...
        # [L,B,D] -> [B,D,L]
        output_feat = output.reshape(
            num_imgs, h, w, batch, dim).permute(0, 3, 4, 1, 2)
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory, pos: Optional[Tensor] = None, query_pos: Optional[Tensor] = None):
        ...
        for layer in self.layers:
            output = layer(output, memory, input_shape=tgt_shape,
