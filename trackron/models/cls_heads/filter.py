...
FILTER_REGISTRY.__doc__ = """
Registry for localization filter, which extract target feature from the feature maps
...
class FilterPool(nn.Module):
    ...
    def __init__(self, *, filter_size=1, feature_stride=16, pool_square=False):
        ...
        Args:
            filter_size (int, optional): [description]. Defaults to 1.
            feature_stride (int, optional): [description]. Defaults to 16.
            pool_square (bool, optional): [description]. Defaults to False.
        """
        super().__init__()
    ...
    def from_config(cls, cfg):
        ...
        return {"filter_size": filter_size, "feature_stride": feature_stride}
...
class FilterInitializer(nn.Module):
    ...
    def __init__(self, *, filter_size=1, feature_dim=256, feature_stride=16, pool_square=False, filter_norm=True,
        ...
        Args:
            filter_size (int, optional): [description]. Defaults to 1.
            feature_dim (int, optional): [description]. Defaults to 256.
            feature_stride (int, optional): [description]. Defaults to 16.
            pool_square (bool, optional): [description]. Defaults to False.
            filter_norm (bool, optional): [description]. Defaults to True.
            num_filter_pre_convs (int, optional): [description]. Defaults to 1.
            num_filter_post_convs (int, optional): [description]. Defaults to 0.
        """
        super().__init__()
    ...
    def from_config(cls, cfg):
        ...
        pool_square = cfg.POOL_SQUARE
        filter_norm = cfg.NORM
        num_filter_pre_conv = cfg.FIL
        num_filter_post_conv = cfg.FILER_NUM_PRE_CONVER_NUM_POST_CONV
        
        return {"filter_size": filter_size, "feature_stride": feature_stride,
                "feature_dim": feature_dim, "pool_square": pool_square,
                "filter_norm": filter_norm, "num_filter_pre_conv": num_filter_pre_conv,
                "num_filter_post_conv": num_filter_post_conv}


    def forward(self, feat, bb, sample_weights=None):
        ...
        if num_images > 1:
            if sample_weights is None:
                weights = torch.mean(weights.reshape(
                    num_images, -1, weights.shape[-3], weights.shape[-2], weights.shape[-1]), dim=0)
            else:
                sample_weights = torch.reshape(
                    sample_weights, (num_images, -1, 1, 1, 1))
                weights = torch.sum(sample_weights * weights.reshape(
                    num_images, -1, *weights.shape[-3:]), dim=0)
...
class FilterInitializerLinear(nn.Module):
    ...
    def __init__(self, *, filter_size=1, feature_dim=256, feature_stride=16, pool_square=False, filter_norm=True,
        ...
        Args:
            ...
            pool_square (bool, optional): [description]. Defaults to False.
            filter_norm (bool, optional): [description]. Defaults to True.
            conv_ksz (int, optional): [description]. Defaults to 3.
            init_weights (str, optional): [description]. Defaults to 'default'.
        """
        super().__init__()
...
def build_filter(cfg):
    """
    Build filter for extract target feature given box and feature maps
    """
    return FILTER_REGISTRY.get(cfg.NAME)(cfg)