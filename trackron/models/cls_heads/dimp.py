...
class DIMP_CLS_HEAD(nn.Module):
  ...
  def from_config(cls, cfg):
    filter_size = cfg.FILTER.FILTER_SIZE
    feature_stride = cfg.FILTER.FEATURE_STRIDE
    filter_initializer = build_filter(cfg.FILTER)
    filter_optimizer = build_dimp_optimizer(cfg.OPTIMIZER)
    feature_extractor = build_extractor(cfg.EXTRACTOR)
    return {
  ...
  def get_filter(self, feat, bb, *args, **kwargs):
    ...
    feat_center = (bb[..., :2] + bb[..., 2:] / 2) / self.feature_stride
