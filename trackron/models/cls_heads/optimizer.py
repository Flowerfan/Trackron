...
__all__ = [
    "DiMPSteepestDescentGN", "PrDiMPSteepestDescentNewton",
    "DIMP_OPTIMIZER_REGISTRY", "build_dimp_optimizer"
]

DIMP_OPTIMIZER_REGISTRY = Registry("DIMP_OPTIMIZER")
DIMP_OPTIMIZER_REGISTRY.__doc__ = """
Registry for DIMP Optimizer, which extract target feature from the feature maps
...
@DIMP_OPTIMIZER_REGISTRY.register
class DiMPSteepestDescentGN(nn.Module):
  ...
  def __init__(self,
    ...
    if mask_act == 'sigmoid':
      ...
      raise ValueError('Unknown activation %s' % mask_act)
  ...
  def from_config(cls, cfg):
    return {
        "num_iter": cfg.ITER,
        "init_step_length": cfg.STEP,
        "init_gauss_sigma": cfg.GAUSS_SIGMA,
        "num_dist_bins": cfg.NUM_DIST_BINS,
        "bin_displacement": cfg.BIN_DISPLACEMENT,
        "mask_init_factor": cfg.MASK_FACOTR,
        "init_filter_reg": cfg.INIT_FILTER_REG,
        "min_filter_reg": cfg.MIN_FILTER_REG,
        "score_act": cfg.SCORE_ACT,
  ...
  def forward(self,
    ...
    """Runs the optimizer module.
        ...
        args:
            ...
            center:  Target center (cx, cy) in the feature map coords. Dims (images_in_sequence, [sequences], 2).
    ...
    dmap_offset = (torch.Tensor(filter_sz).to(center.device) % 2) / 2.0
    center = center.reshape(-1, 2).flip((1,)) - dmap_offset
    ...
    if sample_weight is None:
      ...
      sample_weight = sample_weight.sqrt().reshape(num_images, num_sequences, 1,
                                                   1) * spatial_weight
    ...
    for i in range(num_iter):
      if not backprop_through_learning or (i > 0 and
                                           i % self.detach_length == 0):
        weights = weights.detach()

      # Compute residuals
      scores = filter_layer.apply_filter(feat, weights, padding=padding)
      ...
      scores_grad = filter_layer.apply_filter(feat,
                                              weights_grad,
                                              padding=padding)
      ...
      alpha_den = ((scores_grad * scores_grad).reshape(
    ...
    if compute_losses:
      ...
      losses.append(
          (((sample_weight * (scores - label_map))**2).sum() + reg_weight *
...
class DiMPL2SteepestDescentGN(nn.Module):
  ...
  def get_label(self, center, output_sz):
    ...
    k0 = torch.arange(output_sz[0],
                      dtype=torch.float32).reshape(1, 1, -1,
                                                   1).to(center.device)
    k1 = torch.arange(output_sz[1],
                      dtype=torch.float32).reshape(1, 1, 1,
                                                   -1).to(center.device)
    g0 = torch.exp(-1.0 / (2 * self.gauss_sigma**2) *
                   (k0 - center[:, :, 0].reshape(*center.shape[:2], 1, 1))**2)
    g1 = torch.exp(-1.0 / (2 * self.gauss_sigma**2) *
                   (k1 - center[:, :, 1].reshape(*center.shape[:2], 1, 1))**2)
    gauss = g0 * g1
    return gauss
  ...
  def forward(self,
    ...
    for i in range(num_iter):
      ...
      score_mask = target_mask + (1.0 -
                                  target_mask) * (scores.detach() > 0).float()
      residuals = sample_weight * (scores_act - label_map)
...
class PrDiMPSteepestDescentNewton(nn.Module):
  ...
  def forward(self,
    ...
    for i in range(num_iter):
      ...
      grad_hes_grad = (sample_weight.reshape(sample_weight.shape[0], -1) *
...
class ContraDiMPSteepestDescentNewton(nn.Module):
  """Optimizer module for PrDiMP.
    It unrolls the steepest descent with Newton iterations to optimize the target filter. See the PrDiMP paper.
    args:
        num_iter:  Number of default optimization iterations.
  ...
  #              init_filter_reg=1e-2, gauss_sigma=1.0, num_dist_bins=5, bin_displacement=1.0, mask_init_factor=4.0,
  ...
  def get_label_mask(self, center, output_sz):
    center = center.reshape(center.shape[0], -1, center.shape[-1])
    k0 = torch.arange(output_sz[0],
                      dtype=torch.float32).reshape(1, 1, -1,
                                                   1).to(center.device)
    ...
    if self.gauss_sigma == 0:
      ...
      gauss = (g0 / (2 * math.pi * self.gauss_sigma**2)) * g1
    # gauss = gauss * (gauss > self.label_threshold).float()
    mask = (gauss > self.label_threshold).float()
    return mask
    # if self.normalize_label:
    #     gauss /= (gauss.sum(dim=(-2,-1), keepdim=True) + 1e-8)
  ...
  def forward(self,
    ...
    center = ((bb[..., :2] + bb[..., 2:] / 2) / self.feat_stride).flip(
        (-1,)) - offset
    target_mask = self.get_label_mask(center, output_sz)
    # target_mask = (label > self.label_threshold).float()
    # dist_map = self.distance_map(center, output_sz)
    ...
    def _compute_loss(scores, weights):
      return torch.sum(sample_weight.reshape(sample_weight.shape[0], -1) *
                       (torch.log(scores.exp().sum(dim=(-2, -1)) + exp_reg) - torch.log((scores.exp()*target_mask).sum(dim=(-2, -1))+exp_reg))) / num_sequences +\
             reg_weight * (weights ** 2).sum() / num_sequences
    ...
    for i in range(num_iter):
      ...
      # Compute "residuals"
      scores = filter_layer.apply_filter(feat, weights)
      # shift = scores.mean(dim=(-2,-1), keepdim=True)
      shift = scores.max(-1, keepdim=True)[0].max(-2, keepdim=True)[0]
      scores_exp = (scores - shift).exp()
      scores_softmax = activation.softmax_reg(
          scores.reshape(num_images, num_sequences, -1),
          dim=2,
          reg=self.softmax_reg).reshape(scores.shape)
      pos_softmax = scores_exp * target_mask / (scores_exp * target_mask).sum(
          dim=(-2, -1), keepdim=True).clamp_(1e-5)
      res = sample_weight * (scores_softmax - pos_softmax)
      ...
      scores_grad = filter_layer.apply_filter(feat, weights_grad)
      sm_scores_grad = scores_softmax * scores_grad
      pos_scores_grad = pos_softmax * scores_grad
      hes_sm_scores_grad = sm_scores_grad - scores_softmax * torch.sum(
          sm_scores_grad, dim=(-2, -1), keepdim=True)
      hes_pos_scores_grad = pos_scores_grad - pos_softmax * torch.sum(
          pos_scores_grad, dim=(-2, -1), keepdim=True)
      hes_scores_grad = hes_sm_scores_grad - hes_pos_scores_grad
      grad_hes_grad = (scores_grad * hes_scores_grad).reshape(
          num_images, num_sequences, -1).sum(dim=2).clamp(min=0)
      grad_hes_grad = (sample_weight.reshape(sample_weight.shape[0], -1) *
                       grad_hes_grad).sum(dim=0)
...
class GRUFeat(nn.Module):
  ...
  def forward(self,
    ...
    ## get init weights
    filter_feat = self.filter_pre_layers(
        feat.reshape(-1, feat.shape[-3], feat.shape[-2], feat.shape[-1]))
    weights = self.filter_pool(filter_feat, bb)
    weights = torch.mean(weights.reshape(feat.shape[0], -1, weights.shape[-3],
    ...
    for i in range(num_iter):
      ...
      # Compute gradient
      filter_feat = self.gru(
          filter_feat,
          residuals_mapped.reshape(-1, 1, residuals_mapped.shape[-2],
                                   residuals_mapped.shape[-1]))
      weights = self.filter_pool(filter_feat, bb)
      weights = torch.mean(weights.reshape(feat.shape[0], -1, weights.shape[-3],
                                           weights.shape[-2],
                                           weights.shape[-1]),
...
class MSDiMPSteepestDescentGN(nn.Module):
  """Optimizer module for DiMP.
    It unrolls the steepest descent with Gauss-Newton iterations to optimize the target filter.
    Moreover it learns parameters in the loss itself, as described in the DiMP paper.
    args:
...
def build_dimp_optimizer(cfg):
  return DIMP_OPTIMIZER_REGISTRY.get(cfg.NAME)(cfg)
