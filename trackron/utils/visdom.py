...
class VisCostVolumeUI(VisBase):
  ...
  def shade_cell(self, data):
    ...
    data[:, r1:r2, c1:c2] = data[:, r1:r2, c1:c2] * (1 - factor) + torch.tensor(
...
class VisTracking(VisBase):
  ...
  def draw_data(self):
    ...
    for i, disp_rect in enumerate(boxes):
      ...
      cv2.rectangle(
          disp_image, (int(disp_rect[0]), int(disp_rect[1])),
  ...
  def register(self, data, mode, debug_level=0, title='Data', **kwargs):
    if title not in self.registered_blocks.keys():
      ...
      if mode == 'image':
        ...
        self.registered_blocks[title] = VisHeatmap(self.visdom, show_data,
                                                   title)
      elif mode == 'cost_volume':
        self.registered_blocks[title] = VisCostVolume(self.visdom, show_data,
                                                      title)
        ...
        self.registered_blocks[title] = VisCostVolumeUI(self.visdom, show_data,
                                                        title, data[1],
                                                        self.registered_blocks)
      elif mode == 'info_dict':
        self.registered_blocks[title] = VisInfoDict(self.visdom, show_data,
                                                    title)
      elif mode == 'text':
        self.registered_blocks[title] = VisText(self.visdom, show_data, title)
      elif mode == 'lineplot':
        self.registered_blocks[title] = VisLinePlot(self.visdom, show_data,
                                                    title)
      elif mode == 'Tracking':
        self.registered_blocks[title] = VisTracking(self.visdom, show_data,
                                                    title)
      elif mode == 'bbreg':
        self.registered_blocks[title] = VisBBReg(self.visdom, show_data, title)
      elif mode == 'featmap':
        self.registered_blocks[title] = VisFeaturemap(self.visdom, show_data,
                                                      title)
      else:
